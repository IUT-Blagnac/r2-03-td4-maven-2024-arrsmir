	...
	public static void inverserTableau(int[] pfTableauDeValeurs) {
		...
		for (int i = 0; i < pfTableauDeValeurs.length; i++) {
			try {
				...
				System.out.println("Erreur, impossible d'empiler la pile est deja pleine. " + e.getMessage());
			}
		}
		pos = 0;
		while (!p.estVide()) {
			try {
				...
				System.out.println("Erreur, impossible de recuperer ou de retirer la valeur en haut de pile, la pile est vide. " + e.getMessage());
			}
		}
	}
